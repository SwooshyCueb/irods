...
namespace irods
{
    ...
    const std::string RIPEMD160_NAME("ripemd160");

    error RIPEMD160Strategy::init(boost::any& _context) const
    {
        EVP_MD* message_digest = EVP_MD_fetch(nullptr, RIPEMD160_NAME.c_str(), nullptr);
    ...
    error RIPEMD160Strategy::update(const std::string& data, boost::any& _context) const
    {
    ...
    error RIPEMD160Strategy::digest(std::string& _messageDigest, boost::any& _context) const
    {
        EVP_MD_CTX* context = boost::any_cast<EVP_MD_CTX*>(_context);
        ...
        std::array<unsigned char, RIPEMD160_DIGEST_LENGTH> final_buffer;
        ...
        std::size_t out_len = CHKSUM_LEN - std::strlen(RIPEMD160_CHKSUM_PREFIX);

        std::array<unsigned char, CHKSUM_LEN> out_buffer;
        int rc = base64_encode(final_buffer.data(), RIPEMD160_DIGEST_LENGTH, out_buffer.data(), &out_len);
        if (0 != rc) {
            auto msg = fmt::format("{}: Failed to base64 encode hash.", __func__);
            return ERROR(rc, std::move(msg));
        }

        _messageDigest = RIPEMD160_CHKSUM_PREFIX;
        _messageDigest += std::string(reinterpret_cast<char*>(out_buffer.data()), out_len);

        return SUCCESS();
    }

    bool RIPEMD160Strategy::isChecksum(const std::string& _chksum) const
    {
        return _chksum.starts_with(RIPEMD160_CHKSUM_PREFIX);
    }
}; // namespace irods
