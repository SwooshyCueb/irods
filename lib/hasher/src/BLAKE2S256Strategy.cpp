...
namespace irods
{
    ...
    const std::string BLAKE2S256_NAME("blake2s256");

    error BLAKE2S256Strategy::init(boost::any& _context) const
    {
        EVP_MD* message_digest = EVP_MD_fetch(nullptr, BLAKE2S256_NAME.c_str(), nullptr);
    ...
    error BLAKE2S256Strategy::update(const std::string& data, boost::any& _context) const
    {
    ...
    error BLAKE2S256Strategy::digest(std::string& _messageDigest, boost::any& _context) const
    {
        EVP_MD_CTX* context = boost::any_cast<EVP_MD_CTX*>(_context);

        const std::size_t DIGEST_LENGTH = 256 / 8;
        ...
        std::size_t out_len = CHKSUM_LEN - std::strlen(BLAKE2S256_CHKSUM_PREFIX);
        ...
        _messageDigest = BLAKE2S256_CHKSUM_PREFIX;
        _messageDigest += std::string(reinterpret_cast<char*>(out_buffer.data()), out_len);

        return SUCCESS();
    }

    bool BLAKE2S256Strategy::isChecksum(const std::string& _chksum) const
    {
        return _chksum.starts_with(BLAKE2S256_CHKSUM_PREFIX);
    }
}; // namespace irods
