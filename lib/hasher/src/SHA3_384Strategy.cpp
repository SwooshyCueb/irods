...
namespace irods
{
    ...
    const std::string SHA3_384_NAME("sha3-384");

    error SHA3_384Strategy::init(boost::any& _context) const
    {
        EVP_MD* message_digest = EVP_MD_fetch(nullptr, SHA3_384_NAME.c_str(), nullptr);
    ...
    error SHA3_384Strategy::update(const std::string& data, boost::any& _context) const
    {
    ...
    error SHA3_384Strategy::digest(std::string& _messageDigest, boost::any& _context) const
    {
        EVP_MD_CTX* context = boost::any_cast<EVP_MD_CTX*>(_context);
        ...
        std::array<unsigned char, SHA384_DIGEST_LENGTH> final_buffer;
        ...
        std::size_t out_len = LONG_CHKSUM_LEN - std::strlen(SHA3_384_CHKSUM_PREFIX);

        std::array<unsigned char, LONG_CHKSUM_LEN> out_buffer;
        int rc = base64_encode(final_buffer.data(), SHA384_DIGEST_LENGTH, out_buffer.data(), &out_len);
        if (0 != rc) {
            auto msg = fmt::format("{}: Failed to base64 encode hash.", __func__);
            return ERROR(rc, std::move(msg));
        }

        _messageDigest = SHA3_384_CHKSUM_PREFIX;
        _messageDigest += std::string(reinterpret_cast<char*>(out_buffer.data()), out_len);

        return SUCCESS();
    }

    bool SHA3_384Strategy::isChecksum(const std::string& _chksum) const
    {
        return _chksum.starts_with(SHA3_384_CHKSUM_PREFIX);
    }
}; // namespace irods
