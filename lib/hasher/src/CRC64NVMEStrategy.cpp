...
namespace irods
{
    ...
    // x^64 + x^63 + x^61 + x^59 + x^56 + x^55 + x^52 + x^49 + x^48 + x^47 + x^46 + x^44 + x^41 + x^37 + x^36 + x^34
    ...
    static constexpr std::uint64_t crc_polynomial = 0xAD93D23594C93659;
    ...
    static constexpr std::uint64_t crc_initial_remainder = 0xFFFFFFFFFFFFFFFF;
    ...
    static constexpr std::uint64_t crc_final_xor = 0xFFFFFFFFFFFFFFFF;
    ...
    auto CRC64NVMEStrategy::update(const std::string& data, boost::any& _context) const -> error
    {
        ...
        try {
            ...
            return ERROR(SYS_INVALID_INPUT_PARAM,
                         "The context sent to CRC64NVMEStrategy::update was not a proper boost::crc_optimal<>*");
        }
    ...
    auto CRC64NVMEStrategy::digest(std::string& messageDigest, boost::any& _context) const -> error
    {
        ...
        try {
            ...
            return ERROR(SYS_INVALID_INPUT_PARAM,
                         "The context sent to CRC64NVMEStrategy::digest was not a proper boost::crc_optimal<>*");
        }
