...
namespace irods
{
    ...
    const std::string SHA224_NAME("sha224");

    error SHA224Strategy::init(boost::any& _context) const
    {
        EVP_MD* message_digest = EVP_MD_fetch(nullptr, SHA224_NAME.c_str(), nullptr);
    ...
    error SHA224Strategy::update(const std::string& data, boost::any& _context) const
    {
    ...
    error SHA224Strategy::digest(std::string& _messageDigest, boost::any& _context) const
    {
        EVP_MD_CTX* context = boost::any_cast<EVP_MD_CTX*>(_context);
        ...
        std::size_t out_len = CHKSUM_LEN - std::strlen(SHA224_CHKSUM_PREFIX);
        ...
        _messageDigest = SHA224_CHKSUM_PREFIX;
        _messageDigest += std::string(reinterpret_cast<char*>(out_buffer.data()), out_len);

        return SUCCESS();
    }

    bool SHA224Strategy::isChecksum(const std::string& _chksum) const
    {
        return _chksum.starts_with(SHA224_CHKSUM_PREFIX);
    }
}; // namespace irods
