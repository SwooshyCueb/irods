...
namespace irods
{
    ...
    const std::string SHA384_NAME("sha384");

    error SHA384Strategy::init(boost::any& _context) const
    {
        EVP_MD* message_digest = EVP_MD_fetch(nullptr, SHA384_NAME.c_str(), nullptr);
    ...
    error SHA384Strategy::update(const std::string& data, boost::any& _context) const
    {
    ...
    error SHA384Strategy::digest(std::string& _messageDigest, boost::any& _context) const
    {
        EVP_MD_CTX* context = boost::any_cast<EVP_MD_CTX*>(_context);
        ...
        std::size_t out_len = LONG_CHKSUM_LEN - std::strlen(SHA384_CHKSUM_PREFIX);
        ...
        _messageDigest = SHA384_CHKSUM_PREFIX;
        _messageDigest += std::string(reinterpret_cast<char*>(out_buffer.data()), out_len);

        return SUCCESS();
    }

    bool SHA384Strategy::isChecksum(const std::string& _chksum) const
    {
        return _chksum.starts_with(SHA384_CHKSUM_PREFIX);
    }
}; // namespace irods
